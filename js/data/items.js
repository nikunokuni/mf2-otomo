/* ===========================================================
   ...
   name    アイテム名。狭い画面に合わせて、長いものは short な呼び方にしている
           （カララギマンゴー→マンゴー / オイリーオイル→油 /
             ソンナバナナ→バナナ / 夏見草→草）。
           ★保存データに入るキーなので、変えるときは下の RENAMED_ITEMS に足すこと★
...
export const ITEMS = [
  ...
  // 桃は「その週に与えた」ことを調整ローテに残すためのもの。
  // 若返るぶん（黄金桃+50週 / 白銀桃+25週）は育成計画の桃システムで組むので、
  // ここでは内部数値もパラメーターも動かさない
  {
    name: '黄金桃',
    kind: 'use',
    effect: '若返る（成長段階が戻り、50週ぶん長く育てられる）／内部数値は動かない',
  },
  {
    name: '白銀桃',
    kind: 'use',
    effect: '若返る（成長段階が戻り、25週ぶん長く育てられる）／内部数値は動かない',
  },
  {
    name: '双子の水差し',
    kind: 'hold',
...
/**
 * アイテムの名前を短くしたときの読み替え表（旧名 → 新名）。
 * 名前は保存データに入る（調整ローテの各週・早見に保存したローテ）ので、
 * 読み込みのときに store.js が付け替える。
